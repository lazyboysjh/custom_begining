...
export const Schema = z
  .object({
    世界: z.preprocess(
      ...
      z.object({
        ...
        出场角色: z.preprocess(value => value == null ? [] : value, z.array(z.string()).prefault([])),
  ...
  .transform(data => {
    ...
    if (legacyName && !data.角色[legacyName]) {
      ...
      data.角色[legacyName] = {
        ...legacyRole,
        污秽度: clamp100(legacyRole.污秽度),
        信任: clamp100(legacyRole.信任),
        好感度: clamp100(legacyRole.好感度),
        堕落值: clamp100(legacyRole.堕落值),
        依存度: clamp100(legacyRole.依存度),
      };
    }
